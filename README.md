...
## 복원 과정에서 빠졌던 핵심 기능 복구

**실제로 발견된 누락**: 대화 기록만으로 `app.py`를 다시 짜맞추는 과정에서,
"공고 자동수집" 화면 아래쪽에 있던 **"방금 수집된 공고를 바로 검색·검토하는
영역"을 통째로 빠뜨렸었다** - 사이드바의 "🔍 공고 실시간 검색"(공고제목/특이사항
검색, 발주기관 검색, 검토완료 숨기기 체크박스)과, 그 결과를 "🌟 주요 4대 중앙
공고"(한국시설안전협회·조달청 통합명부·아이건설넷·나라장터) / "📋 일반 기관 공고"
두 그룹으로 나눠 보여주는 부분, 그리고 "✅ 현재 화면 전체 일괄 검토완료" 버튼까지
다시 채워넣었다.

**덤으로 발견한 버그**: 사이드바 "최근 실행" 표시가 항상 비어있었다 - 원인은
`manage_sheet_lock(doc, "unlock")`이 실행이 끝날 때마다 `settings` 탭의
시각/엔진명까지 통째로 지워버리고 있었기 때문. "최근 실행"은 실행이 끝난
뒤에도 계속 보여야 하는 정보이므로, unlock 시 상태(A1)만 "free"로 바꾸고
시각/엔진명(B1/C1)은 그대로 남기도록 고쳤다. 또한 `get_recent_log()`가
GitHub Actions API만 보고 있어서 클라우드에서 실행한 기록은 아예 못 잡았던
것도, `settings` 탭을 직접 읽도록 고쳐서 클라우드/사무실PC 어느 쪽으로
실행했든 정확히 반영되게 했다.
