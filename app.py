...
menu = st.sidebar.radio("이동할 메뉴를 선택하세요:", ["공고 자동수집", "공고 통계", "🎯 타겟 공고", "📝 게시판"])
st.sidebar.divider()
...
st.sidebar.link_button("💧 한수원 K-Pro", "https://ebiz.khnp.co.kr/login.do")
...
if menu == "공고 자동수집":
    ...
    with st.container(border=True):
        ...
        st.divider()
        engine_choice = st.radio("⚙️ 수집 엔진 선택", ["빠른 탐색(열람가능 사이트)", "정밀 탐색(셀레니움 병행)"], horizontal=True)
        ...
        if st.button("🚀 공고 수집 시작", type="primary", use_container_width=True):
            if not scan_mode and selected_orgs_str == "ALL":
                ...
                st.warning("⏳ 현재 다른 팀원이 공고를 수집 중입니다.")
            else:
                target_script = "main_pure.py" if "빠른" in engine_choice else "main.py"
                ...
                with st.status(f"🚀 [{target_script}] 수집 엔진 가동 중...", expanded=True) as status:
                    try:
                        ...
                        # 🚀 Python 스크립트에 파라미터 전달 (기간, 키워드, 특정기관목록)
    ...
    if not df.empty and '공고제목' in df.columns:
        ...
        search_keyword = st.sidebar.text_input("공고제목 검색", "")
        search_org = st.sidebar.text_input("발주기관(출처) 검색", "")
        hide_reviewed = st.sidebar.checkbox("✅ 검토 완료 숨기기", value=True)
        ...
        st.subheader(f"📋 공고 검색 결과 ({len(filtered_df)}건)")
        render_notice_table(filtered_df, "general")
    else:
        st.info("아직 구글 시트에 수집된 데이터가 없습니다.")

# (공고 통계, 타겟 공고, 게시판 메뉴 등 기존과 동일하게 작동하도록 간략화 유지)
elif menu == "공고 통계":
    st.title("📊 공고 통계")
    df = get_google_sheet("notices")
    if not df.empty:
        st.metric("총 누적 수집 공고", f"{len(df)} 건")
    ...
    if not df.empty and '검토유무' in df.columns:
        ...
        st.dataframe(df_my, hide_index=True)

elif menu == "📝 게시판":
    st.title("📝 팀 게시판 및 메모장")
    df_memos = get_google_sheet("memos")
    if not df_memos.empty: st.dataframe(df_memos, use_container_width=True)
