...
#
# 실제로 발견된 문제: 대전 상수도사업본부처럼 하루에 공고가 10건 넘게 올라오는
# 게시판은, 수집 기간(예: 15일)만큼 다 훑기도 전에 이 상한선(예전 값 8)에
# 걸려서 "아직 오래된 지점을 못 만났는데도" 조용히 멈춰버리는 바람에, 수집
# 기간 안에 있는 공고를 놓치는 실제 사례가 있었다. 이 값을 넉넉하게 올려서,
# 대부분의 저용량 사이트는 원래대로 1~3페이지 안에 자연스럽게 멈추고, 이
# 상한선은 정말 페이지가 끝없이 이어지는 등 진짜 오작동 상황에서만 발동하는
# '최후의 안전장치'로만 쓰이게 한다.
